...
def main(args):
    ...
    for idx in tqdm.tqdm(imgidx):
        ...
        fp.write('%s\t%d\n'%(filename, label))
        cv2.imwrite(os.path.join(args.output_dir, filename), img, [int(cv2.IMWRITE_JPEG_QUALITY), 100])
    fp.close()
    shutil.copy(os.path.join(args.root_dir, 'agedb_30.bin'), os.path.join(args.output_dir, 'agedb_30.bin'))
    shutil.copy(os.path.join(args.root_dir, 'cfp_fp.bin'), os.path.join(args.output_dir, 'cfp_fp.bin'))
    shutil.copy(os.path.join(args.root_dir, 'lfw.bin'), os.path.join(args.output_dir, 'lfw.bin'))
