...
parser.add_argument(
    "--log_dir", type=str, default="./logs/", required=True)
parser.add_argument(
...
def extract_info_from_file(log_file, result_dict, speed_dict):
    ...
    with open(log_file) as f:
        ...
        for line in lines:
            if "Speed" in line:
                p1 = re.compile(r"Speed ([0-9]+\.[0-9]+) samples/sec", re.S)
    ...
    speed_dict[mode][model][run_case][precision][batch_size_per_device][
        test_iter] = avg_speed
    average_speed = compute_average(speed_dict[mode][model][run_case][precision][
...
def compute_speedup(result_dict, speed_dict):
    ...
    for md in mode_list:
        ...
        for m in model_list:
            run_case = [key for key in result_dict[md][m]]  # eg.['4n8g', '2n8g', '1n8g', '1n4g', '1n1g']
            for d in run_case:
                precision = [key for key in result_dict[md][m][d]]
                for p in precision:
                    batch_size_per_device = [key for key in result_dict[md][m][d][p]]
                    for b in batch_size_per_device:
                        speed_up = 1.0
                        if result_dict[md][m]['1n1g'][p][b]['median_speed']:
                            speed_up = result_dict[md][m][d][p][b][
                                'median_speed'] / result_dict[md][m]['1n1g'][p][b][
                                    'median_speed']
                        result_dict[md][m][d][p][b]['speedup'] = round(speed_up, 2)
...
def extract_result():
    ...
    result_file_name = os.path.join(args.output_dir, "arcface_torch_result.json")
